...
from nonebot.plugin import on_command, on_request, on_message
from nonebot.rule import to_me
from nonebot.adapters.onebot.v11 import MessageSegment, Event, FriendRequestEvent, PrivateMessageEvent
...
@bot_data.handle()
async def bot_data(event: Event):
    send_id = event.get_user_id()
    code = await bot_start()
    if pic_msg:
        if code == 200:
            message = 'Bot数据加载成功！'
            pict = await pic(message)
        ...
        if code == 200:
            if send_id in super_id:
                await nonebot.get_bot().send(user_id=send_id, message='Bot数据加载成功！', at_sender=True, event=event)
        else:
            if send_id in super_id:
                await nonebot.get_bot().send(user_id=send_id, message='Bot数据加载失败！', event=event, at_sender=True)


# 查看机器人好友列表
check_friend_list = on_command('查看bot好友列表', aliases={'查看机器人好友列表', '查看好友列表'}, permission=SUPERUSER)


@check_friend_list.handle()
async def check_friend_list(event: Event):
    send_id = event.get_user_id()
...
async def check_group_list(event: Event):
    send_id = event.get_user_id()
...
@formsg.handle()
async def formsg(event: PrivateMessageEvent):
    ...
    try:
        if to_su:
            if sent_id not in super_id:
                ...
                for su in super_id:
                    await nonebot.get_bot().send_msg(message_type='private', user_id=int(su), message=message,
                                                     at_sender=True)
        else:
            pass
    except Exception as e:
        for su in super_id:
            await nonebot.get_bot().sent_msg(message_type='private', user_id=int(su), message=f'错误信息{e}',
                                             at_sender=True)


# 关闭私聊信息转给超管
formsg_close = on_command('私聊转发关', aliases={'消息转发关', 'formsg_close'}, permission=SUPERUSER)


@formsg_close.handle()
async def for_msg_close(event: Event):
    send_id = event.get_user_id()
    global to_su
    to_su = False
    if send_id in super_id:
...
# 开启私聊转发给超管
formsg_open = on_command('私聊转发开', aliases={'消息转发开', 'formsg_open'}, permission=SUPERUSER)


@formsg_open.handle()
async def formsg_open(event: Event):
    send_id = event.get_user_id()
    global to_su
    to_su = True
    if send_id in super_id:
...
async def add_friend(event: FriendRequestEvent):
    try:
        ...
        flag = add_req['flag']
        realtime = time.strftime('%Y年%m月%d日 %H:%M:%S', time.localtime(add_req['time']))  # 请求添加时间
        ...
        if len(obj) == 0:
            ...
            if pic_msg:
                pict = await pic(message)
                for su in super_id:
                    await nonebot.get_bot().send_msg(message_type='private', user_id=int(su),
        ...
        if pic_msg:
            ...
            for su in super_id:
                await nonebot.get_bot().send_msg(message_type='private', user_id=int(su), message=f'机器人出错了！\n错误信息：{e}',
                                                 at_sender=True)
...
async def agree_add_friend(event: Event):
    send_id = event.get_user_id()
    try:
...
async def refuse_add_friend(event: Event):
    send_id = event.get_user_id()
    try:
        agree_id = int(str(event.get_message()).split('#')[-1])
        with open(path + '/add_friend.json', 'r', encoding='utf-8') as f:
...
@add_qq_list.handle()
async def add_qq_list(event: Event):
    send_id = event.get_user_id()
    try:
        with open(path + '/add_friend.json', 'r', encoding='utf-8') as f:
            obj = json.load(f)
...
async def delete_friend(event: Event):
    send_id = event.get_user_id()
    try:
...
@sent_msg.handle()
async def sent_msg(event: Event):
    send_id = event.get_user_id()
    try:
        ...
        for item in obj:
            if sent_id == int(item['user_id']):
                ...
                if pic_msg:
                    ...
                    await nonebot.get_bot().send_msg(message_type='private', user_id=sent_id, message=message,
                                                     at_sender=True)
                    if send_id in super_id:
                        await nonebot.get_bot().send(user_id=send_id, message=MessageSegment.image(pict), event=event,
                                                     at_sender=True)
                    ...
                    await nonebot.get_bot().send_msg(message_type='private', user_id=sent_id, message=message,
                                                     at_sender=True)
                    if send_id in super_id:
                        await nonebot.get_bot().send(user_id=send_id, message=message1, event=event, at_sender=True)
                mark = True
                break
        if not mark:
            message = f'失败！\nQQ:{sent_id}不在好友列表中。'
...
@sent_msg_all.handle()
async def sent_msg_all(event: Event):
    send_id = event.get_user_id()
    msg = '群发易封号，慎用！！！！！！'
    if pic_msg:
        pict = await pic(msg)
        ...
        if send_id in super_id:
            await nonebot.get_bot().send(user_id=send_id, message=msg, event=event, at_sender=True)
    try:
        with open(path + '/friend_list.json', 'r', encoding='utf-8') as f:
            obj = json.load(f)
        message1 = '已成功发送好友列表\n'
...
@bot_status.handle()
async def bot_status(event: Event):
    send_id = event.get_user_id()
    try:
        ...
        lastmessagetime = time.strftime('%Y年%m月%d日 %H:%M:%S', time.localtime(statistics['stat']['LastMessageTime']))
        ...
        if not mark:
            ...
            if area == '湖北':
                data = {'qq': qq, 'area': area, 'openid': openid, 'uid': uid, 'name': name, 'danwei1': danwei1,
                        'danwei2': danwei2, 'danwei3': danwei3, 'class_name': class_name}
                obj.append(data)
                ...
                if mark1:
                    data = {'qq': qq, 'area': area, 'openid': openid, 'uid': uid, 'name': name, 'danwei1': danwei1,
                            'danwei2': danwei2, 'danwei3': danwei3, 'nid': nid, 'class_name': class_name}
                    obj.append(data)
                    with open(path + '/dxx_list.json', 'w', encoding='utf-8') as f:
                        json.dump(obj, f, ensure_ascii=False, indent=4)
                    code = await start_use_jx(send_id)
